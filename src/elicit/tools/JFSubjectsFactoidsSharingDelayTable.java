 ...
 * @author mmanso
 */
public class JFSubjectsFactoidsSharingDelayTable extends JFSubjectsFactoidsTable {

    public JFSubjectsFactoidsSharingDelayTable(TrialData trialData) {
        super(trialData);
    }
    ...
    {
        this.setTitle("Factoids Sharing or Post Delay");
    }
        ...
        for (TrialData.Subject s : m_trialData.m_organizationInformation.m_memberList)
        {
            for ( int i=1; i<=m_trialData.m_factoidStats.totalFactoids ; i++ )
            {
                ...
                if (hShared!=null && hShared.m_time<=time) {
                    jtFactoids.setValueAt(hShared.m_time - hHoarded.m_time, sCount,  i);
